  ...
  return (
    <div className="flex-1 px-5 pb-6">
      ...
      <div className="pt-1 pb-6 text-center">
        ...
        <div
          className="font-display leading-none text-[#1A1A1E]"
          style={{ fontSize: 84, marginTop: 12 }}
        >
          {credoScore}
        </div>
        <div className="mt-2 text-[14px] font-medium text-[#6B6B73]">
          {getTierLabel(credoScore, "credo")}
        </div>
        <div
          className="mt-1 text-[12px] font-medium"
          style={{ color: deltaColor(scores.credo.delta) }}
        >
      ...
      <div className="grid grid-cols-4 gap-2">
        {(Object.keys(PILLARS) as PillarKey[]).map((key) => {
          ...
          return (
            <div
              key={key}
              className="rounded-[14px] border border-[#E5E5E8] bg-white px-1.5 py-3 text-center"
            >
              <div
                className="mx-auto mb-1.5 h-1 w-6 rounded-full"
                style={{ background: pillar.color }}
              />
              <div className="text-[11px] font-medium text-[#6B6B73]">
                {pillar.label}
              </div>
              <div className="mt-0.5 font-mono text-[20px] font-bold leading-tight text-[#1A1A1E]">
                {Math.round(s.score)}
              </div>
              <div
                className="text-[11px] font-medium"
                style={{ color: deltaColor(s.delta) }}
              >
          ...
          <div className="flex items-start justify-between px-4 pb-3 pt-4">
            <div>
              <div className="flex items-baseline gap-2.5">
                <span className="font-mono text-[32px] font-bold leading-none text-[#1A1A1E]">
              ...
              <div className="mt-1 text-[13px] font-medium text-[#6B6B73]">
                {getTierLabel(strength.overall, "strength")}
              </div>
            </div>
            <div className="text-right">
              <div className="font-mono text-[16px] font-semibold text-[#1A1A1E]">
                {ordinal(strength.percentile)}
              </div>
              <div className="text-[11px] text-[#9E9EA3]">percentile</div>
            </div>
          </div>
          <div className="border-t border-[#E5E5E8] bg-[#F7F7F8] px-4 py-2 text-[12px] text-[#6B6B73]">
          ...
          <div className="border-t border-[#E5E5E8]">
            {strength.subscores.map((s) => {
              ...
              return (
                <div
                  key={s.category}
                  className="border-b border-[#E5E5E8] px-4 py-3 last:border-b-0"
                >
                  <div className="flex items-baseline justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-[11px] text-[#9E9EA3]">
                        {s.keyLift}
                      </div>
                    </div>
                    {hasData && (
                      <div className="shrink-0 text-right">
                        <div className="font-mono text-[14px] font-semibold text-[#1A1A1E]">
                  ...
                  {hasData ? (
                    ...
                    <div className="mt-1.5 text-[12px] text-[#9E9EA3]">
      ...
      <div className="mt-6">
        <button
          ...
          className="w-full rounded-[12px] border border-[#E5E5E8] bg-transparent py-2.5 text-[13px] font-semibold text-[#6B6B73] transition-colors hover:border-[#9E9EA3] hover:text-[#1A1A1E] disabled:opacity-50"
        >
          {recalculating ? "Recalculating…" : "Recalculate scores"}
        </button>
        {recalcError && (
          <div className="mt-2 text-center text-[12px] font-medium text-[#C43B3B]">
            {recalcError}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex-1 px-5 pb-6">
      <div className="flex flex-col items-center pt-4">
        ...
        <div className="mt-5 h-20 w-28 animate-pulse rounded-[14px] bg-[#EEEFF1]" />
        <div className="mt-4 h-3 w-24 animate-pulse rounded bg-[#EEEFF1]" />
      </div>
      <div className="mt-7 grid grid-cols-4 gap-2">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="h-[92px] animate-pulse rounded-[14px] bg-[#EEEFF1]" />
        ))}
      </div>
      <div className="mt-7 h-64 animate-pulse rounded-[14px] bg-[#EEEFF1]" />
      <div className="mt-7 h-80 animate-pulse rounded-[14px] bg-[#EEEFF1]" />
    </div>
  );
}
