...
function DeltaText({ delta }: { delta: number }) {
  const color = delta > 0 ? "text-success" : delta < 0 ? "text-danger" : "text-text-tertiary";
  const sign = delta > 0 ? "+" : "";
  return (
  ...
  return (
    <div className="flex flex-col gap-5 px-5 pb-6">
      <div className="pt-2 text-center">
        ...
        <h1 className="mt-1 text-xl font-semibold text-text-primary">{focus}</h1>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {stats.map((s) => (
          <div
            key={s.label}
            className="rounded-[14px] border border-app bg-card-surface px-3 py-3 text-center"
          >
            <p className="font-mono text-lg font-semibold text-text-primary">{s.value}</p>
            <p className="mt-0.5 text-[11px] font-medium text-text-secondary uppercase">
              {s.label}
            </p>
          </div>
        ))}
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {personalRecords.map((pr, i) => (
            <motion.div
              ...
              className="flex items-center gap-3 rounded-[14px] border-2 border-credo bg-credo-light px-4 py-3.5"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-credo text-white">
                <Trophy size={18} />
              </div>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="rounded-[14px] border border-app bg-card-surface px-4 py-1">
          <div className="flex items-center justify-between border-b border-app py-3">
            <span className="text-sm font-semibold text-text-primary">Credo score</span>
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between py-3 ${
                  i < pillarKeys.length - 1 ? "border-b border-app" : ""
                }`}
              >
                <span className="text-sm text-text-secondary">{PILLARS[key].label}</span>
                <div className="flex items-center gap-2">
      ...
      <Link
        href="/app/dashboard"
        className="focus-ring flex h-12 items-center justify-center rounded-[12px] bg-credo text-[15px] font-semibold text-white transition-colors hover:bg-credo/90"
      >
        Back to dashboard
      </Link>
    </div>
